...
export async function getFileState(
  ...
  const { db } = await getLibraryDatabase(library)
  const rows = await db.select().from(fileState).where(eq(fileState.path, path))
  return rows[0] ?? null
}

export async function listFileStates(
  library: Library,
...
export async function clearFileStatesForLibrary(
  library: Library,
): Promise<void> {
  const { db } = await getLibraryDatabase(library)
