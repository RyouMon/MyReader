...
function toRemoteEntryPath(source: WebDavDataSource, href: string, isDirectory: boolean) {
  ...
  if (normalizedPath === basePath || normalizedPath === `${basePath}/`) return "";
...
function parsePropfind(source: WebDavDataSource, xml: string) {
  ...
  return responses
    .map((chunk) => {
      ...
      return { href: remotePath, path: remotePath, name: displayName || fallbackName, isDirectory };
    })
...
export class WebDavRemoteBackend implements RemoteBackend {
    ...
    try {
      ...
      if (!response.ok) {
        throw new NetworkError(
          i18n.t("sync.webdavPropfindFailed", { status: response.status, path: remotePath }),
          response.status,
        );
      }
  ...
  async readBytes(remotePath: string): Promise<Uint8Array> {
    ...
    if (!response.ok) {
      throw new NetworkError(
        i18n.t("sync.webdavGetFailed", { status: response.status, path: remotePath }),
        response.status,
      );
    }
  ...
  async writeBytes(remotePath: string, bytes: Uint8Array): Promise<void> {
    ...
    const response = await fetch(this.urlBuilder.urlFor(remotePath), {
      method: "PUT",
      headers: { ...this.urlBuilder.authHeaders, "Content-Type": "application/octet-stream" },
      body: bytes as unknown as BodyInit,
    });
    if (!response.ok) {
      throw new NetworkError(
        i18n.t("sync.webdavPutFailed", { status: response.status, path: remotePath }),
        response.status,
      );
    }
  }
  ...
  async deleteRemote(remotePath: string): Promise<void> {
    ...
    if (!response.ok && response.status !== 404) {
      throw new NetworkError(
        i18n.t("sync.webdavDeleteFailed", { status: response.status, path: remotePath }),
        response.status,
      );
    }
  }
  ...
  async listRemote(prefix: string): Promise<string[]> {
    ...
    while ((match = hrefRegex.exec(xml)) !== null) {
      ...
      const childName = hrefPath.endsWith("/") ? `${childRaw.replace(/\/$/, "")}/` : childRaw;
  ...
  async listDirectory(path: string): Promise<RemoteDirEntry[]> {
    ...
    return parsePropfind(this.source, xml).filter((entry) => {
  ...
  private async ensureParentDirectories(relativePath: string): Promise<void> {
    ...
    for (let i = 0; i < parts.length - 1; i += 1) {
      ...
      if (!response.ok && ![201, 301, 405].includes(response.status)) {
        throw new NetworkError(
          i18n.t("sync.webdavMkcolFailed", { status: response.status, path: cursor }),
          response.status,
        );
      }
    }
