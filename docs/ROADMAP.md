<div align="right"><a href="./ROADMAP_EN.md">English</a></div>

# MyReader Roadmap

> 路线图与跨平台支持矩阵。这里同时记录已实现、部分可用和规划中的能力；只有标为 ✅ 的项目才是当前公开能力。路线图表示方向，不代表发布日期或交付承诺。
...
|---|---|
| ✅ | 已实现并在 UI 暴露 |
| ⚠️ | 底层已实现、部分可用或仍属试验性，但 UI 未暴露 / 平台受限 / 只读 / 稳定性待验证 |
| ❌ | 未实现 |
| 🗑️ | 曾在类型或旧代码中存在，已移除 |
| — | 不适用 |
...
[ADR-0016](./adr/0016-adopt-automerge-for-library-sidecar-sync.md) 与
[ARCHITECTURE.md](./ARCHITECTURE.md)。当前以每个书库的 sidecar 为独立同步边界，不建立跨书库的中央用户数据库。

| 阶段 | 状态 | 目标 |
|---|---|---|
| 书库 sidecar 数据库 | ✅ | 进度、书签等书库域数据随书库保存 |
| 书库与本地副本身份 | ✅ | `library_uuid` 标识同一书库文档；每份设备本地 sidecar 拥有独立 `replica_id` |
| Automerge 书库增量同步 | ⚠️ | 已同步收藏、进度、书签、批注、阅读会话、完成记录；MyReader 书库另同步书目。当前仍属试验性功能，可能出现未知问题 |
...
| 能力 | 状态 | 当前边界 |
|---|---|---|
| TTS 与听读衔接 | ❌ | 尚无内置朗读或可选 TTS 引擎 |
| ComfyUI 创意生成 | ❌ | 尚无选段出图、图生图、漫画参考或视频生成流程 |
