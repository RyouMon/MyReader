...
const mobileTokenPath = path.join(
  repoRoot,
  "my-reader-mobile/src/design/tokens.tsx",
)
const readerTokenPath = path.join(
  repoRoot,
  "my-reader-mobile/src/design/reader-tokens.ts",
)
...
function syncMobileTokens(mobileTokens, rootVars, darkVars) {
  ...
  const borderValueLight = getTokenValue(
    rootVars,
    darkVars,
    "--border",
    "light",
  ...
  const borderStrongLight = getTokenValue(
    rootVars,
    darkVars,
    "--border-strong",
    "light",
  )
  const borderStrongDark = getTokenValue(
    rootVars,
    darkVars,
    "--border-strong",
    "dark",
...
function syncReaderTokens(readerTokens, rootVars, darkVars) {
  ...
  output = output.replace(
    /(paper:\s*\{[\s\S]*?link:\s*)"[^"]*"/m,
    `$1"${primaryLight}"`,
  )
  output = output.replace(
    /(light:\s*\{[\s\S]*?link:\s*)"[^"]*"/m,
    `$1"${primaryLight}"`,
  )
  output = output.replace(
    /(dark:\s*\{[\s\S]*?link:\s*)"[^"]*"/m,
 ...
 */
async function main() {
  const [sourceCss, designDoc, desktopCss, mobileTokens, readerTokens] =
    await Promise.all([
  ...
  const darkVars = parseCssVars(
    extractCssBlock(sourceCss, '[data-theme="dark"]'),
  )
