...
pub fn library_book_file_path(lib: &LibraryConfig, app_data_dir: &Path, relative: &str) -> PathBuf {
...
pub fn compute_book_relative_path(file_path: &Path, lib_root: &Path) -> Result<String, AppError> {
    file_path
...
mod tests {
    ...
    fn library_book_file_path_should_resolve_under_root_path() {
        ...
        assert_eq!(
            library_book_file_path(&webdav_library(), &app_data, "Stephen King/It/cover.jpg"),
    ...
    fn compute_book_relative_path_should_convert_backslashes() {
        ...
        let lib_root = PathBuf::from("app-data").join("libraries").join("lib-1");
        let file_path = lib_root.join("Author").join("Title").join("book.epub");
        assert_eq!(
            compute_book_relative_path(&file_path, &lib_root).unwrap(),
            "Author/Title/book.epub"
        );
