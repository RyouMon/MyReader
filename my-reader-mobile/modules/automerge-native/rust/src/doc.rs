...
impl Doc {
    ...
    pub fn get_in_map(&self, obj: ObjId, key: String) -> Result<Option<Value>, DocError> {
        ...
        assert_map(&*doc, &obj)?;
        let result = doc.get(obj, key)?;
        Ok(result.map(Value::from))
    }
    ...
    pub fn splice(
        ...
        doc.splice(
            ...
            values
                .into_iter()
                .map(|value| am::hydrate::Value::from(am::ScalarValue::from(value))),
        )?;
        Ok(())
    }
    ...
    pub fn changes_since(&self, heads: Vec<ChangeHash>) -> Vec<Change> {
        let heads = heads
            .into_iter()
            .map(am::ChangeHash::from)
            .collect::<Vec<_>>();
    ...
    pub fn missing_deps(&self, heads: Vec<ChangeHash>) -> Vec<ChangeHash> {
        let heads = heads
            .into_iter()
            .map(am::ChangeHash::from)
            .collect::<Vec<_>>();
...
mod tests {
    ...
    #[test]
    fn should_return_individual_changes_when_querying_since_heads() {
        let doc = Doc::new();
        doc.put_in_map(
            root(),
            "first".to_string(),
        ...
        .unwrap();
        doc.commit_with(None, 1);
        let first_heads = doc.heads();
        ...
        .unwrap();
        doc.commit_with(None, 2);

        let changes = doc.changes_since(first_heads);
    ...
    #[test]
    fn should_report_external_heads_when_document_is_missing_changes() {
        let source = Doc::new();
        source
            .put_in_map(
                root(),
