...
These tables are **excluded from Drizzle migrations** — `drizzle.config.ts` points at `index.ts` which only exports the three app-owned tables.
...
|---------|-------------|
| `pnpm db:generate` | `drizzle-kit generate` + regenerate SeaORM entities (app tables only) |
...
| Export | Contents |
|--------|---------|
| `@my-reader/db/schema` | Drizzle table definitions for app-owned tables (`readingProgress`, `fileState`, `syncMeta`) |
| `@my-reader/db/schema/calibre` | Drizzle table definitions for Calibre mirror schema (`books`, `authors`, link tables, etc.) |
| `@my-reader/db/types` | Inferred TypeScript types for app-owned tables (`ReadingProgress`, `FileState`, `SyncMeta`, etc.) |
