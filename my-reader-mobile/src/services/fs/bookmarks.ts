...
function mapBookmarkErrorToChinese(action: string, error: unknown) {
  ...
  if (
    isNativeErrorMessage(error, /Unsupported URI scheme for bookmark creation/i)
  ) {
    return i18n.t("securityBookmarks.unsupportedUrl", { action })
  }

  if (
    isNativeErrorMessage(
      error,
      /Unable to start accessing security-scoped resource/i,
    )
  ) {
...
export async function createSecurityScopedBookmark(uri: string) {
  ...
  try {
    return await securityScopedBookmarksModule.createBookmarkForDirectoryAsync(
      uri,
    )
  } catch (error) {
    throw createBookmarkOperationError(
      i18n.t("securityBookmarks.createBookmark"),
      error,
    )
  }
}
...
export async function withSecurityScopedLibraryAccess<T>(
  ...
  try {
    ...
    throw createBookmarkOperationError(
      i18n.t("securityBookmarks.accessDirectory"),
      error,
    )
  }
  ...
  try {
    if (access.stale) {
      try {
        ...
        throw createBookmarkOperationError(
          i18n.t("securityBookmarks.refreshBookmark"),
          error,
        )
      }
    }
