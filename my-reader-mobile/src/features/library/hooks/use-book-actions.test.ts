...
function buildFileStateBundle(rows: Record<string, { path: string; localState: string }[]> = {}) {
...
describe("useBookActions", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.spyOn(bookFormats, "getReadableFormats").mockImplementation((formats?: string[]) =>
      (formats ?? []).map((f) => f.toUpperCase()).filter((f) => ["EPUB", "PDF", "CBZ"].includes(f)).sort(),
    );
  ...
  describe("handleBookMenuAction", () => {
    ...
    it("should do nothing when favorite action is invoked and toggle handler is missing", () => {
      ...
      expect(() => result.current.handleBookMenuAction("1", "favorite")).not.toThrow();
    });
    ...
    it("should show alert when reading format paths fails with non-error value", async () => {
      ...
      expect(showAlertWithStatusBarRestore).toHaveBeenCalledWith(expect.any(String), "Disk error");
    });

    it("should do nothing when single readable format resolves to undefined", async () => {
      jest.mocked(bookFormats.getReadableFormats).mockReturnValue(new Array(1) as string[]);
      jest.mocked(getBookFormatPaths).mockResolvedValue([
        { format: "EPUB", relativePath: "Author/Test Book/Test Book.epub" },
      ]);
    ...
    it("should invoke setFormat from default format prompt", async () => {
      ...
      const buttons = jest.mocked(showAlertWithStatusBarRestore).mock.calls[0]![2] as
    ...
    it("should do nothing when setting default format with no setFormat handler", () => {
      ...
      expect(() => result.current.handleBookMenuAction("1", "setDefaultFormat:EPUB")).not.toThrow();
    });
    ...
    it("should do nothing on delete when library is missing", () => {
      const { result } = renderHook(() =>
        useBookActions(
          ...
          buildMetaMap(),
          buildFileStateBundle({
            "1": [{ path: "Author/Test Book/Test Book.epub", localState: "present" }],
          }),
          null,
          {},
          null,
    ...
    it("should do nothing on delete when no downloaded rows exist", () => {
      const { result } = renderHook(() =>
        useBookActions(
          ...
          buildMetaMap(),
          buildFileStateBundle({
            "1": [{ path: "Author/Test Book/Test Book.epub", localState: "remote_only" }],
          }),
          null,
          {},
          remoteLibrary,
    ...
    it("should show alert when sharing throws a non-error value", async () => {
      ...
      expect(showAlertWithStatusBarRestore).toHaveBeenCalledWith(expect.any(String), "String error");
    });
    ...
    it("should share a specific downloaded format when format action is invoked", async () => {
      ...
      expect(resolveShareableFormat).toHaveBeenCalledWith(localLibrary, 1, "EPUB");
      expect(shareBookFile).toHaveBeenCalledWith("file:///tmp/Test%20Book.epub", "EPUB");
    });
    ...
    it("should show alert when sharing throws an error", async () => {
      jest.mocked(resolveShareableFormat).mockRejectedValue(new Error("Disk error"));
    ...
    it("should do nothing when unknown action is invoked", () => {
      ...
      expect(() => result.current.handleBookMenuAction("1", "unknown")).not.toThrow();
    });
    ...
    it("should do nothing when book is not found", () => {
      ...
      expect(() => result.current.handleBookMenuAction("unknown", "detail")).not.toThrow();
      expect(router.push).not.toHaveBeenCalled();
    });
  });
});
