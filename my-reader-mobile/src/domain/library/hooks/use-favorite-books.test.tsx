...
describe("useFavoriteBooks", () => {
  ...
  it("returns favorite ids present in the current books", async () => {
    jest.mocked(listFavoriteBooks).mockResolvedValue([
      { bookId: 1 },
      { bookId: 2 },
      { bookId: 99 },
  ...
  it("adds a favorite when toggling a non-favorite book", async () => {
    jest.mocked(listFavoriteBooks).mockResolvedValue([] as Awaited<ReturnType<typeof listFavoriteBooks>>);
