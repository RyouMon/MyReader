...
type NormalizedImageColors = AndroidImageColors | IOSImageColors | WebImageColors;
...
function buildMaterialPalette(dominantColor: string, scheme: "light" | "dark"): CoverMaterialPalette {
...
function extractUriAndHeaders(input: ImageColorsInput): { uri: string; headers?: Record<string, string> } {
...
async function fetchCoverPalette(input: ImageColorsInput): Promise<CoverPalette | undefined> {
...
): { material: CoverMaterialPalette | undefined; raw: CoverRawColors | undefined } {
  const [palette, setPalette] = useState<CoverPalette | undefined>();

  const schemePalette = useMemo(() => {
    if (!palette) return undefined;
    return colorScheme === "dark" ? palette.material.dark : palette.material.light;
  }, [palette, colorScheme]);
