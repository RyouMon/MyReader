...
async function seedLibrary() {
  ...
  if (existingDir.exists) {
    ...
    if (metadataFile.exists) {
      ...
      const { registerLibrary } = await import(
        "@/src/domain/library/hooks/library-actions"
      )
  ...
  for (const [relativePath, data] of Object.entries(entries)) {
    ...
    if (data.length === 0 && strippedPath.endsWith("/")) {
      ...
      const parentPath = strippedPath.substring(
        0,
        strippedPath.lastIndexOf("/") + 1,
      )
      if (parentPath) {
