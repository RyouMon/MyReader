...
const GENESIS_BYTES: &[u8] = include_bytes!("fixtures/automerge-genesis.automerge");
#[cfg(test)]
const TYPESCRIPT_POSITION_INCREMENTAL: &[u8] =
    include_bytes!("fixtures/typescript-position.incremental");
...
fn parse_replica_actor(replica_id: &str) -> Result<ActorId, AppError> {
    let uuid =
...
fn map_object_id(doc: &AutoCommit, key: &str) -> Result<automerge::ObjId, AppError> {
...
pub fn validate_library_sidecar_document(doc: &AutoCommit) -> Result<(), AppError> {
    let schema = doc
        .get(ROOT, "schema")
...
pub fn load_library_sidecar_document(replica_id: &str) -> Result<AutoCommit, AppError> {
...
pub fn validate_library_identity(doc: &AutoCommit, library_uuid: &str) -> Result<(), AppError> {
    for (value, _) in doc
...
pub fn reading_position_candidates(
    ...
    book_id: i64,
    format: &str,
) -> Result<Vec<ReadingPositionCandidate>, AppError> {
    let positions = map_object_id(doc, "positions")?;
...
pub fn reading_position_projections(
    doc: &AutoCommit,
) -> Result<Vec<ReadingPositionProjection>, AppError> {
    let positions = map_object_id(doc, "positions")?;
...
pub fn favorite_projections(doc: &AutoCommit) -> Result<Vec<(i64, FavoriteValue)>, AppError> {
...
fn bookmark_key(value: &BookmarkValue) -> Result<String, AppError> {
    if value.book_id < 1
...
pub fn set_bookmark(doc: &mut AutoCommit, value: &BookmarkValue) -> Result<ChangeHash, AppError> {
    parse_replica_actor(&value.replica_id)?;
    put_json_register(
        doc,
        "bookmarks",
...
pub fn bookmark_projections(doc: &AutoCommit) -> Result<Vec<BookmarkValue>, AppError> {
...
fn annotation_object(doc: &AutoCommit, id: &str) -> Result<automerge::ObjId, AppError> {
...
fn annotation_i64(doc: &AutoCommit, object: &automerge::ObjId, key: &str) -> Result<i64, AppError> {
    doc.get(object, key)
        .map_err(|error| sync_error(format!("Failed to read annotation {key}: {error}")))?
...
pub fn annotation_projections(doc: &AutoCommit) -> Result<Vec<AnnotationValue>, AppError> {
    let annotations = map_object_id(doc, "annotations")?;
    let mut values = doc
        .keys(&annotations)
        .map(|id| {
...
fn validate_compact_id(id: &str, name: &str) -> Result<(), AppError> {
...
fn document_replica_id(doc: &AutoCommit) -> Result<String, AppError> {
...
) -> Result<Option<ChangeHash>, AppError> {
...
pub fn reading_session_projections(doc: &AutoCommit) -> Result<Vec<ReadingSessionValue>, AppError> {
    let sessions = map_object_id(doc, "sessions")?;
    let mut values = doc
        .keys(&sessions)
        .map(|id| {
...
pub fn reading_completion_records(
    doc: &AutoCommit,
) -> Result<Vec<ReadingCompletionValue>, AppError> {
    let completions = map_object_id(doc, "completions")?;
    let mut values = doc
        .keys(&completions)
        .map(|id| {
