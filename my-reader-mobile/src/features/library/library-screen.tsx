...
import { EmptyState, PrimaryButton, Screen } from "@/src/components"
import { importBookFromPicker } from "@/src/domain/library/hooks/library-actions"
...
import {
  defaultSortOptionForCollection,
  useBooksForCollection,
  type SortOption,
} from "@/src/features/library/hooks/use-books-for-collection"
...
export default function LibraryScreen({ collectionId }: LibraryScreenProps) {
  ...
  const effectiveLibraryId = activeLibraryId ?? undefined
  const {
    data: books = [],
  ...
  const [sortBy, setSortBy] = useState<SortOption>(() =>
    defaultSortOptionForCollection(collectionId),
  )
  ...
  const visibleBooks = useBooksForCollection({
    books,
    recentlyReadBooks,
    collectionId,
  ...
  const displayedBooks = useMemo(
    () =>
      collectionId === "all"
        ...
        : visibleBooks,
    [collectionId, pendingBookImports, visibleBooks],
  )
    ...
    switch (collectionId) {
      case "recentlyRead":
        return {
          ...
          icon: { ios: "clock.arrow.circlepath", android: "history" } as const,
        }
      case "favorites":
        return {
  ...
  const flashList = (
    <FlashList
      key={`${collectionId}-${viewMode}-${gridColumns}-${activeLibraryId ?? "none"}`}
