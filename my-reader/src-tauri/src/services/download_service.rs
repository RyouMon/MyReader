...
impl DownloadService {
    ...
    async fn get_stored_file_state(
        ...
        Ok(
            my_reader_core::api::content::ContentService::get_file_state(sidecar_root, path)
                .await?,
        )
    }
    ...
    pub async fn check_file_states(
        ...
        let rows_by_path = if lib.is_remote() {
            let sidecar_root = library_sidecar_path(&lib, app_data_dir);
            my_reader_core::api::content::ContentService::get_file_states(
                &sidecar_root,
                &relative_paths,
            )
            .await?
    ...
    async fn download_book_file_inner<R: Runtime>(
        ...
        let remote_path = my_reader_core::api::content::ContentService::resolve_remote_file_path(
            source_path,
            book_relative_path,
        )?;
        info!(
