...
export async function injectTauriInternals(target: Page | BrowserContext) {
  await target.addInitScript(() => {
    const existingHandlers = (window as unknown as Record<string, Record<string, IpcHandler>>).__TAURI_IPC_HANDLERS__
    ...
    window.__TAURI_INTERNALS__ = {
      invoke: async (cmd: string, args: Record<string, unknown>) => {
        const currentHandlers = (window as unknown as Record<string, Record<string, IpcHandler>>).__TAURI_IPC_HANDLERS__ ?? {}
      ...
      transformCallback: (
        ...
        const callbacks =
        // @ts-expect-error - __TAURI_INTERNALS__ is not defined in the window object
          ((window.__TAURI_INTERNALS__ as unknown as Record<string, unknown>).callbacks as
        ...
        // @ts-expect-error - __TAURI_INTERNALS__ is not defined in the window object
        ;(window.__TAURI_INTERNALS__ as unknown as Record<string, unknown>).callbacks = callbacks
        return id
      },
      metadata: {
