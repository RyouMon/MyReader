...
fn progress_dto(
    library_id: &str,
    value: myreader_core::models::ReadingPosition,
) -> ReadingProgressDto {
    ReadingProgressDto {
...
impl ProgressService {
    pub async fn get_reading_progress_for_library(
        ...
        Ok(
            myreader_core::api::reading::get_reading_position(&sidecar_root, book_id, format)
                .await?
    ...
    pub async fn list_reading_progress_for_library(
        ...
        let sidecar_root = library_sidecar_path(&library, app_data_dir);
        Ok(
            myreader_core::api::reading::list_reading_positions(&sidecar_root)
                .await?
                .into_iter()
                .map(|value| progress_dto(&library.id, value))
                .collect(),
