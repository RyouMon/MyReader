...
export function useBookDetailFormats(
  ...
  const [formatInfoMap, setFormatInfoMap] = useState<
    Record<string, FormatInfo>
  >({})
  ...
  const { data: fileStateRows = EMPTY_FILE_STATE_ROWS } =
    useFileStates(activeLibrary)
      ...
      void getBookFormatPaths(activeLibrary, detail.id)
        .then((paths) => {
          ...
          for (const { format, relativePath } of paths) {
            const file = new File(
              libraryBookFileUri(activeLibrary, relativePath),
            )
            const isPresent = file.exists && (file.size ?? 0) > 0
            map[format] = {
              relativePath,
              localState: isPresent ? "present" : null,
            }
          }
          if (cancelled) return
  ...
  useEffect(() => {
    ...
    const relevantTasks = downloadStatusTasks.filter(
      (task) =>
        ...
        (task.bookId === bookId ||
          Object.values(latestMap).some(
            (info) => info.relativePath === task.relativePath,
          )),
    )
    ...
    const doneTasks = relevantTasks.filter(
      (task) =>
        ...
        !deletedLocalPathKeysRef.current.has(
          `${task.libraryId}${task.relativePath}`,
        ),
    )
    ...
    setFormatInfoMap((prev) => {
      ...
      for (const task of doneTasks) {
        ...
        if (
          current?.localState === "present" &&
          current.relativePath === task.relativePath
        )
          continue
        next[format] = {
  ...
  const handleDownloadFormat = useCallback(
    async (format: string) => {
      ...
      if (!info || !detail) {
        Alert.alert(
          t("bookDetail.cannotStartDownload"),
          t("bookDetail.noFormatPath", { format }),
        )
        return
      }
      try {
        ...
        deletedLocalPathKeysRef.current.delete(
          `${activeLibrary.id}${info.relativePath}`,
        )
      } catch (err) {
  ...
  const handleShareFormat = useCallback(
    async (format: string) => {
      if (!detail) return
      const shareable = await resolveShareableFormat(
        activeLibrary,
        detail.id,
        format,
      )
