...
import type { DataSource, DataSourceOnedrive, DataSourceType, DataSourceWebdav } from "@my-reader/tools/types/data-source";
...
export async function writeSecrets(id: string, secrets: DataSourceSecrets): Promise<void> {
  switch (secrets.type) {
    case "webdav":
...
export async function deleteSecrets(id: string, type: DataSourceType): Promise<void> {
  switch (type) {
    case "webdav":
...
export function deriveCredentialFlags(secrets?: DataSourceSecrets): Record<string, boolean> {
  if (!secrets) return {};
  switch (secrets.type) {
    case "webdav":
