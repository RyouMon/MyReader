...
function buildCloseAction(
  ...
  const fallbackRoute = (close.target ??
    "/settings") as import("expo-router").RelativePathString
...
export function useScreenHeader({
  ...
  const androidOptions = useMemo((): NativeStackNavigationOptions => {
    ...
    if (headerLargeTitle !== undefined)
      options.headerLargeTitle = headerLargeTitle
    if (headerShadowVisible !== undefined)
      options.headerShadowVisible = headerShadowVisible
  ...
  const iosToolbarLeftActions = useMemo(():
    | ScreenHeaderAction[]
    | undefined => {
  ...
  const iosOptions = useMemo((): NativeStackNavigationOptions => {
    ...
    if (useLayoutClose) {
      options.headerLeft = () => (
        <HeaderCloseButton
          fallbackRoute={
            (close?.target ?? "/settings") as import("expo-router").Href
          }
