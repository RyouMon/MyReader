...
//!
//! 本模块是阶段 1 目标的聚合点：
//! - [`backend`]：基于 OpenDAL 的后端工厂，支持 WebDAV 与 LocalDirect（本地直读）与 OneDrive。
...
/// - `Onedrive` 的 access_token 在此为空；真正需要时由
///   [`backend::build_operator`] 通过 token manager 懒刷新。
