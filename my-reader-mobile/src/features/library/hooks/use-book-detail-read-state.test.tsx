import { act, renderHook as baseRenderHook } from "@testing-library/react-native";
...
function renderReadState(overrides: Partial<ReadStateOptions> = {}) {
  const {
    ...
    formatInfoMap = { EPUB: { relativePath: "book.epub", localState: "present" } as FormatInfo },
...
describe("useBookDetailReadState", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    const { isReadableInAppFormat, pickReadableFormat } = jest.requireMock("@my-reader/tools/utils");
  ...
  describe("format selection", () => {
    ...
    it("should filter formats with isReadableInAppFormat", () => {
      const { isReadableInAppFormat } = jest.requireMock("@my-reader/tools/utils");
      isReadableInAppFormat.mockImplementation((format: string) => format === "EPUB");
  ...
  describe("remote library", () => {
    ...
    it("should start download and alert when selected format is not present", () => {
      ...
      const { result } = renderReadState({
        activeLibrary: remoteLibrary,
        formatInfoMap: { EPUB: { relativePath: "book.epub", localState: "remote_only" } as FormatInfo },
        handleDownloadFormat,
      });
