...
final class ReaderService: Loggable {
  ...
  init(config: StreamerConfig = .shared) {
    ...
    // Open the Streamer: default parsers always handle EPUB/PDF/CBZ. The
    // `onCreatePublication` hook (REP-005) and `contentProtections` (REP-006)
    // come from StreamerConfig — Phase 1 no-ops, Phase 2 dispatches registered
    // transforms (CSS/JS injection, SearchService, custom services) and LCP.
    self.publicationOpener = PublicationOpener(
      parser: parser,
      contentProtections: config.contentProtections,
      onCreatePublication: { _, _, _ in
        // REP-005 onCreatePublication hook — Phase 2 applies
        // StreamerConfig.transforms to mutate the manifest, root container,
        // or service factories.
      }
    )
