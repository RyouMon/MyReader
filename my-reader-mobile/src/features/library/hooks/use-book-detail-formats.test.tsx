...
jest.mock("@/src/services/fs/library-paths", () => ({
  libraryBookFileUri: jest.fn((library: Library, relativePath: string) =>
...
describe("useBookDetailFormats", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.mocked(useFileStates).mockReturnValue({ data: [] } as unknown as ReturnType<typeof useFileStates>);
    mockFormatPaths();
  });
  ...
  describe("local library", () => {
    ...
    it("should alert when reading local format paths fails", async () => {
      ...
      await renderHook(() => useBookDetailFormats(localLibrary, "1", detail), { wrapper });
    ...
    it("should stringify non-Error when reading local format paths fails", async () => {
      ...
      await waitFor(() => {
        expect(alertSpy).toHaveBeenCalledWith(expect.any(String), "disk failure");
      });
    });

    it("should ignore local format path updates after unmount", async () => {
      const { promise, resolve } = createDeferred<
        Array<{ format: string; relativePath: string }>
      >();
      ...
      unmount();
      resolve([{ format: "EPUB", relativePath: "Author/Test Book/Test Book.epub" }]);
      await act(async () => {});
    });
  ...
  describe("remote library", () => {
    ...
    it("should alert when reading remote format paths fails", async () => {
      const alertSpy = jest.spyOn(Alert, "alert").mockImplementation(() => {});
      jest.mocked(getBookFormatPaths).mockRejectedValue(new Error("Network error"));

      await renderHook(() => useBookDetailFormats(remoteLibrary, "1", detail), { wrapper });
    ...
    it("should stringify non-Error when reading remote format paths fails", async () => {
      ...
      await waitFor(() => {
        expect(alertSpy).toHaveBeenCalledWith(expect.any(String), "network failure");
      });
    });
  ...
  describe("download task status", () => {
    it("should update format to present when download task completes", async () => {
      ...
      await waitFor(() => {
        expect(result.current.formatInfoMap.EPUB?.localState).toBe("remote_only");
      });
    ...
    it("should match download task by relative path when bookId differs", async () => {
      ...
      const { result } = await renderHook(() => useBookDetailFormats(remoteLibrary, "1", detail), { wrapper });
    ...
    it("should preserve existing relativePath when done task reports a different path", async () => {
      ...
      await waitFor(() => {
        expect(result.current.formatInfoMap.EPUB?.localState).toBe("present");
        expect(result.current.formatInfoMap.EPUB?.relativePath).toBe("Author/Test Book/Test Book.epub");
      });
    });
    ...
    it("should ignore done task when format cannot be determined", async () => {
      ...
      await waitFor(() => {
        expect(result.current.formatInfoMap).toEqual({
          EPUB: { relativePath: "Author/Test Book/Test Book.epub", localState: null },
          PDF: { relativePath: "Author/Test Book/Test Book.pdf", localState: null },
        });
      });
    });
  ...
  describe("handleDownloadFormat", () => {
    ...
    it("should alert when enqueue fails", async () => {
      ...
      jest.mocked(downloadStore.enqueue).mockRejectedValue(new Error("Enqueue failed"));
  ...
  describe("handleDeleteFormat", () => {
    ...
    it("should update format state and consume done tasks on delete confirm", async () => {
      ...
      jest.mocked(confirmDeleteLocalDownload).mockImplementation((_title, _libraryId, _path, callbacks) => {
  ...
  describe("handleShareFormat", () => {
    it("should share local file", async () => {
      jest.mocked(shareBookFileModule.resolveShareableFormat).mockResolvedValue({
        format: "EPUB",
        relativePath: "Author/Test Book/Test Book.epub",
      ...
      expect(shareBookFileModule.shareBookFile).toHaveBeenCalledWith("file:///tmp/Test%20Book.epub", "EPUB");
    });
    ...
    it("should show alert when shareable format cannot be resolved", async () => {
      const alertSpy = jest.spyOn(Alert, "alert").mockImplementation(() => {});
      jest.mocked(shareBookFileModule.resolveShareableFormat).mockResolvedValue(null);
    ...
    it("should ignore user cancellation when sharing", async () => {
      ...
      jest.mocked(shareBookFileModule.shareBookFile).mockRejectedValue(new Error("User cancelled"));
    ...
    it("should alert when sharing fails", async () => {
      ...
      jest.mocked(shareBookFileModule.shareBookFile).mockRejectedValue(new Error("Share failed"));
    ...
    it("should alert when sharing fails with a non-Error rejection", async () => {
      ...
      jest.mocked(shareBookFileModule.shareBookFile).mockRejectedValue("plain failure");
