...
pub(super) enum ContentRequest {
    ...
    UpsertFileState {
        sidecar_root_path: String,
        path: String,
        update: my_reader_core::models::FileStateUpdate,
    },
    DeleteFileState {
        sidecar_root_path: String,
        path: String,
    ...
    UpsertCoverThumbnailCache {
        sidecar_root_path: String,
        patch: my_reader_core::models::BookCoverThumbnailCachePatch,
    },
...
pub(super) enum ContentResponse {
    ...
    SetReadingFormat(()),
    GetFileState(Option<my_reader_core::models::FileState>),
    ListFileStates(Vec<my_reader_core::models::FileState>),
    UpsertFileState(()),
    DeleteFileState(()),
    FinalizeDownloadedFile(my_reader_core::models::DownloadedFile),
    MarkFileRemoteOnly(()),
    ListCoverThumbnailCache(Vec<my_reader_core::models::BookCoverThumbnailCache>),
    UpsertCoverThumbnailCache(()),
    DeleteCoverThumbnailCache(()),
    ClearCoverThumbnailCache(()),
}

pub(super) fn handle(request: ContentRequest) -> Result<ContentResponse, CoreFfiError> {
    Ok(match request {
        ContentRequest::ListReadingFormats {
            sidecar_root_path,
            library_root_path,
        } => ContentResponse::ListReadingFormats(run_core_async(
            my_reader_core::api::content::list_reading_formats(
        ...
        ContentRequest::ListCoverThumbnailCache {
            ...
            my_reader_core::api::content::list_cover_thumbnail_cache(
                Path::new(&sidecar_root_path),
                &thumbnail_version,
                width_px,
                height_px,
        ...
        ContentRequest::ClearCoverThumbnailCache { sidecar_root_path } => {
            ContentResponse::ClearCoverThumbnailCache(run_core_async(
                my_reader_core::api::content::clear_cover_thumbnail_cache(Path::new(
                    &sidecar_root_path,
                )),
            )?)
        }
