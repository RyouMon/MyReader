...
export function ProgressBar({ progress, rounded = true, color }: ProgressBarProps) {
  ...
  return (
    <View className={`h-2 overflow-hidden ${radiusClass}`} style={{ backgroundColor: palette.backgroundSecondary }}>
      <View
        ...
        style={{ backgroundColor: color ?? palette.primary, width: `${Math.max(0, Math.min(progress, 1)) * 100}%` }}
      />
    </View>
  );
}
